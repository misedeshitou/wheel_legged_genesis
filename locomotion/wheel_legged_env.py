...
class WheelLeggedEnv:
    def __init__(
        ...
        self.scene.add_entity(
            gs.morphs.URDF(file="assets/terrain/plane/plane.urdf", fixed=True)
        )
        # init roboot quat and pos
        match robot_morphs:
            case "urdf":
                self.base_init_pos = torch.tensor(
                    self.env_cfg["base_init_pos"]["urdf"], device=self.device
                )
            case "mjcf":
                self.base_init_pos = torch.tensor(
                    self.env_cfg["base_init_pos"]["mjcf"], device=self.device
                )
            case _:
                self.base_init_pos = torch.tensor(
                    self.env_cfg["base_init_pos"]["urdf"], device=self.device
        ...
        self.height_field = cv2.imread(
            "assets/terrain/png/" + self.terrain_cfg["train"] + ".png",
            cv2.IMREAD_GRAYSCALE,
        )
        self.terrain_height = (
            torch.tensor(self.height_field, device=self.device) * self.vertical_scale
        )
        terrain_pos = []
        if self.terrain_cfg["terrain"]:
            ...
            if self.mode:
                ...
                height_field = cv2.imread(
                    "assets/terrain/png/" + self.terrain_cfg["eval"] + ".png",
                    cv2.IMREAD_GRAYSCALE,
                )
                self.terrain = self.scene.add_entity(
                    morph=gs.morphs.Terrain(
        ...
        if self.terrain_cfg["vertical_stairs"]:
            ...
            inverted_pyramid_size = self.v_plane_size + self.v_stairs_width * 2 * (
                self.v_stairs_num * 2
            )
            inverted_pyramid_point = [
                inverted_pyramid_size,
                -inverted_pyramid_size / 2,
                0,
            ]
            self.add_inverted_pyramid(inverted_pyramid_point)
            pyramid_size = self.v_plane_size + self.v_stairs_width * 2 * (
                self.v_stairs_num - 1
            )
        ...
        # dof limits
        lower = [
            self.env_cfg["dof_limit"][name][0] for name in self.env_cfg["joint_names"]
        ]
        upper = [
            self.env_cfg["dof_limit"][name][1] for name in self.env_cfg["joint_names"]
        ]
        ...
        self.rew_survive = torch.ones(
            self.num_envs, dtype=torch.float, device=self.device, requires_grad=False
        )

        # 存活比例
        self.survive_ratio = torch.zeros(
            (self.num_envs,), device=self.device, dtype=gs.tc_float
        )

        # prepare command_ranges lin_vel_x lin_vel_y ang_vel height_target
        self.command_ranges = torch.zeros(
            (self.num_envs, self.num_commands, 2), device=self.device, dtype=gs.tc_float
        )
        ...
        self.lin_vel_error = torch.zeros(
            self.num_envs, device=self.device, dtype=gs.tc_float
        )
        self.ang_vel_error = torch.zeros(
            self.num_envs, device=self.device, dtype=gs.tc_float
        )
        # self.height_error = torch.zeros((self.num_envs,1), device=self.device, dtype=gs.tc_float)
        self.linx_vel_err_range_err = (
            self.curriculum_cfg["lin_vel_err_range"][1]
            - self.curriculum_cfg["lin_vel_err_range"][0]
        )
        self.ang_vel_err_range_err = (
        ...
        init_dof_pos_list = [
            [
                self.env_cfg["joint_init_angles"][name]
                for name in self.env_cfg["joint_names"]
            ]
        ] * self.num_envs
        ...
        self.left_foot_idx = self.left_foot.idx_local
        self.right_foot_idx = self.right_foot.idx_local
        ...
        self.friction_ratio_range = (
            self.domain_rand_cfg["friction_ratio_range"][1] - self.friction_ratio_low
        )
        self.base_mass_low = self.domain_rand_cfg["random_base_mass_shift_range"][0]
        self.base_mass_range = (
            self.domain_rand_cfg["random_base_mass_shift_range"][1] - self.base_mass_low
        )
        self.other_mass_low = self.domain_rand_cfg["random_other_mass_shift_range"][0]
        self.other_mass_range = (
            self.domain_rand_cfg["random_other_mass_shift_range"][1]
            - self.other_mass_low
        )
        self.dof_damping_low = self.domain_rand_cfg["damping_range"][0]
        self.dof_damping_range = (
            self.domain_rand_cfg["damping_range"][1] - self.dof_damping_low
        )
        self.dof_armature_low = self.domain_rand_cfg["dof_armature_range"][0]
        self.dof_armature_range = (
            self.domain_rand_cfg["dof_armature_range"][1] - self.dof_armature_low
        )
    ...
    def _resample_commands(self, envs_idx):
        if self.command_cfg["high_speed"]:
            for command_idx in (0, 1):
                ...
                self.commands[envs_idx, command_idx] = gs_rand_float(
                    low, high, envs_idx.shape, self.device
                )
            ...
            angv_high = torch.clip(
                self.command_ranges[envs_idx, 2, 1],
                torch.zeros(envs_idx.shape, dtype=torch.float32, device=self.device),
                angv_limit,
            )
            self.commands[envs_idx, 2] = gs_rand_float(
                angv_low, angv_high, envs_idx.shape, self.device
            )
            ...
            self.commands[envs_idx, 5] = gs_rand_normal(
                0, tsk_std, envs_idx.shape, self.device
            )
            self.commands[envs_idx, 5] = torch.clip(
                self.commands[envs_idx, 5],
                self.command_cfg["tsk_range"][0],
                self.command_cfg["tsk_range"][1],
            )
            # 角速度命令高就要限制腿部相似 随机一个mean然后高斯采样
            leg_length_mean = gs_rand_float(
            ...
            self.commands[envs_idx, 3] = gs_rand_normal(
                leg_length_mean, leg_length_std, envs_idx.shape, self.device
            )
            self.commands[envs_idx, 4] = gs_rand_normal(
                leg_length_mean, leg_length_std, envs_idx.shape, self.device
            )
            self.commands[envs_idx, 3] = torch.clip(
                self.commands[envs_idx, 3],
                self.command_cfg["leg_length_range"][0],
                self.command_cfg["leg_length_range"][1],
            )
            self.commands[envs_idx, 4] = torch.clip(
                self.commands[envs_idx, 4],
                self.command_cfg["leg_length_range"][0],
                self.command_cfg["leg_length_range"][1],
            )
        else:
    ...
    def set_commands(self, envs_idx, commands):
        self.commands[envs_idx] = torch.tensor(
            commands, device=self.device, dtype=gs.tc_float
        )
    ...
    def step(self, actions):
        ...
        target_dof_vel = (
            exec_actions[:, self.wheel_dof_idx_np] * self.env_cfg["wheel_action_scale"]
        )
        # dof limits
        ...
        self.base_lin_acc[:] = (
            self.base_lin_vel[:] - self.last_base_lin_vel[:]
        ) / self.dt
        self.base_ang_vel[:] = transform_by_quat(self.robot.get_ang(), inv_base_quat)
        self.base_ang_acc[:] = (
            self.base_ang_vel[:] - self.last_base_ang_vel[:]
        ) / self.dt
        ...
        self.left_foot_pos[:] = self.left_foot.get_pos()
        self.right_foot_pos[:] = self.right_foot.get_pos()
        self.left_foot_base_pos[:] = (
            transform_by_quat(self.left_foot_pos, inv_base_quat) + self.base_pos
        )
        self.right_foot_base_pos[:] = (
            transform_by_quat(self.right_foot_pos, inv_base_quat) + self.base_pos
        )
        ...
        if self.noise["use"]:
            self.base_ang_vel[:] += (
                ...
                + (torch.rand_like(self.base_ang_vel) * 2 - 1)
                * self.noise["ang_vel"][1]
            )
            self.projected_gravity += (
                torch.randn_like(self.projected_gravity) * self.noise["gravity"][0]
                + (torch.rand_like(self.projected_gravity) * 2 - 1)
                * self.noise["gravity"][1]
            )
            self.dof_pos[:] += (
        ...
        self.slice_obs_buf = torch.cat(
            [
                ...
                (
                    self.dof_pos[:, self.joint_dof_idx_np]
                    - self.default_dof_pos[:, self.joint_dof_idx_np]
                )
        ...
        self.obs_buf = torch.cat(
            [self.history_obs_buf, self.slice_obs_buf.unsqueeze(1)], dim=1
        ).view(self.num_envs, -1)
        # Update history buffer
        if self.history_length > 1:
            self.history_obs_buf[:, :-1, :] = self.history_obs_buf[
                :, 1:, :
            ].clone()  # 移位操作
        self.history_obs_buf[:, -1, :] = self.slice_obs_buf

        self.obs_buf = torch.cat(
            [self.obs_buf, self.commands * self.commands_scale], axis=-1
        )
    ...
    def reset_idx(self, envs_idx):
        ...
        if self.mode:  # 仅在训练模式 (train_mode=True) 下进行随机化
        ...
        # 计算要重置的环境步数
        self.survive_ratio = (
            self.episode_length_buf[envs_idx].float().mean() / self.max_episode_length
        )
        ...
        if self.terrain_cfg["terrain"]:
            if self.mode:
                ...
                terrain_idx = envs_idx[
                    terrain_buf.nonzero(as_tuple=False).flatten()
                ]  # 获取 envs_idx 中满足条件的索引
                non_terrain_idx = envs_idx[
                    (~terrain_buf).nonzero(as_tuple=False).flatten()
                ]  # 获取 envs_idx 中不满足条件的索引
                # 设置地形位置
                if len(terrain_idx) > 0:  # 只有当有满足地形重置条件的环境时才执行
                    ...
                    )  # 注意从 1 开始，避免使用 base_terrain_pos[0] 作为随机位置
    ...
    def check_termination(self):
        ...
        if self.survive_ratio > 0.8:
            self.reset_buf |= (
                ...
                > self.env_cfg["termination_if_pitch_greater_than"][1]
            )
            self.reset_buf |= (
                torch.abs(self.base_euler[:, 0])
                > self.env_cfg["termination_if_roll_greater_than"][1]
            )
        else:
            self.reset_buf |= (
                torch.abs(self.base_euler[:, 1])
                > self.env_cfg["termination_if_pitch_greater_than"][0]
            )
            self.reset_buf |= (
                torch.abs(self.base_euler[:, 0])
                > self.env_cfg["termination_if_roll_greater_than"][0]
            )
        ...
        if self.env_cfg["termination_if_base_connect_plane_than"]:
            for idx in self.reset_links:
                self.reset_buf |= (
                    torch.abs(self.connect_force[:, idx, :]).sum(dim=1) > 0
                )
    ...
    def domain_rand(self, envs_idx):
        ...
        friction_ratio = (
            ...
            + self.friction_ratio_range * torch.rand(len(envs_idx), self.robot.n_links)
        )
        ...
        base_mass_shift = self.base_mass_low + self.base_mass_range * torch.rand(
            len(envs_idx), 1, device=self.device
        )
        other_mass_shift = -self.other_mass_low + self.other_mass_range * torch.rand(
            len(envs_idx), self.robot.n_links - 1, device=self.device
        )
        ...
        base_com_shift = -self.domain_rand_cfg[
            "random_base_com_shift"
        ] / 2 + self.domain_rand_cfg["random_base_com_shift"] * torch.rand(
            len(envs_idx), 1, 3, device=self.device
        )
        other_com_shift = -self.domain_rand_cfg[
            "random_other_com_shift"
        ] / 2 + self.domain_rand_cfg["random_other_com_shift"] * torch.rand(
            len(envs_idx), self.robot.n_links - 1, 3, device=self.device
        )
        ...
        kp_shift = (
            self.kp_low
            + self.kp_range * torch.rand(len(envs_idx), self.num_actions, device="cpu")
        ) * self.kp[0]
        self.robot.set_dofs_kp(kp_shift, self.motors_dof_idx, envs_idx=envs_idx)

        kv_shift = (
            self.kv_low
            + self.kv_range * torch.rand(len(envs_idx), self.num_actions, device="cpu")
        ) * self.kv[0]
        self.robot.set_dofs_kv(kv_shift, self.motors_dof_idx, envs_idx=envs_idx)

        # random_default_joint_angles
        dof_pos_shift = self.joint_angle_low + self.joint_angle_range * torch.rand(
            len(envs_idx), self.num_actions, device=self.device, dtype=gs.tc_float
        )
        self.default_dof_pos[envs_idx] = dof_pos_shift + self.basic_default_dof_pos

        damping = self.dof_damping_low + self.dof_damping_range * torch.rand(
            len(envs_idx), self.robot.n_dofs
        )
        damping[:, :6] = 0
        ...
        armature = self.dof_armature_low + self.dof_armature_range * torch.rand(
            len(envs_idx), self.robot.n_dofs
        )
        armature[:, :6] = 0
    ...
    def curriculum_commands(self):
        ...
        self.mean_lin_vel_error = (
            (self.lin_vel_error / self.episode_length_buf).mean().item()
        )
        self.mean_ang_vel_error = (
            (self.ang_vel_error / self.episode_length_buf).mean().item()
        )
    ...
    def add_pyramid(self, point):
        max_z_pos = (
            self.v_stairs_num * self.v_stairs_height
            - self.v_stairs_height / 2
            + point[2]
        )
        current_z = max_z_pos
    ...
    def add_inverted_pyramid(self, point):
        ...
        for num_stairs in range(self.v_stairs_num * 2):
            for i in range(4):
                ...
                box = self.scene.add_entity(
                    morph=gs.morphs.Box(
                        pos=tuple(box_pos[i]), size=tuple(box_size[i]), fixed=True
                    )
                )
    ...
    def _reward_tracking_lin_x_vel(self):
        ...
        lin_vel_reward = torch.exp(
            -lin_vel_error / self.reward_cfg["tracking_linx_sigma"]
        )
        if self.command_cfg["zero_stable"]:
            near_zero_mask = (self.commands[:, 0] >= -0.001) & (
                self.commands[:, 0] <= 0.001
            )
            if torch.any(near_zero_mask):
                second_error = torch.square(self.base_lin_vel[near_zero_mask, 0])
                second_reward = torch.exp(
                    -second_error / self.reward_cfg["tracking_linx_sigma"]
                )
    ...
    def _reward_tracking_ang_vel(self):
        ...
        ang_vel_reward = torch.exp(
            -ang_vel_error / self.reward_cfg["tracking_ang_sigma"]
        )
        if self.command_cfg["zero_stable"]:
            near_zero_mask = (self.commands[:, 0] >= -0.01) & (
                self.commands[:, 0] <= 0.01
            )
            if torch.any(near_zero_mask):
                second_error = torch.square(self.base_lin_vel[near_zero_mask, 2])
                second_reward = torch.exp(
                    -second_error / self.reward_cfg["tracking_ang_sigma"]
                )
    ...
    def _reward_joint_action_rate(self):
        ...
        joint_action_rate = (
            self.last_actions[:, self.joint_dof_idx_np]
            - self.actions[:, self.joint_dof_idx_np]
        )
    ...
    def _reward_wheel_action_rate(self):
        # Penalize changes in actions
        wheel_action_rate = (
            self.last_actions[:, self.wheel_dof_idx_np]
            - self.actions[:, self.wheel_dof_idx_np]
        )
    ...
    def _reward_feet_distance(self):
        ...
        reward = torch.clip(
            self.reward_cfg["feet_distance"][0] - feet_distance, 0, 1
